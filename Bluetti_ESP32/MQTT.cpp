...
#define LOW_BATTERY_THRESHOLD 90   // Switch to wall outlet
#define HIGH_BATTERY_THRESHOLD 95  // Switch back to charging station

bool usingChargingStation = true;
...
// -------------- HELPER FOR LOGGING IN publishTopic() -------------------
String getFriendlyFieldName(enum field_names field) {
  switch (field) {
    ...
    // Add more cases for all your enum values...

    default: return map_field_name(field); // fallback to original mapping
  }
}

void controlRelayBasedOnBattery(String percentStr) {
    int percent = percentStr.toInt();

    if (usingChargingStation && percent < LOW_BATTERY_THRESHOLD) {
        // Battery is too low, switch to wall outlet
        digitalWrite(RELAY_PIN, HIGH);
        usingChargingStation = false;
        Serial.println("Switching to wall outlet: battery too low");
    }
    else if (!usingChargingStation && percent >= HIGH_BATTERY_THRESHOLD) {
        // Battery is full enough, switch back to charging station
        digitalWrite(RELAY_PIN, LOW);
        usingChargingStation = true;
        Serial.println("Switching to charging station: battery charged");
    }
    else {
        // No change; keep current power source
        Serial.println("No relay change; continuing with current power source");
    }
}
...
void publishTopic(enum field_names field_name, String value){
  ...
  if (field_name == TOTAL_BATTERY_PERCENT) {
    ...
    controlRelayBasedOnBattery(batteryPercent);
